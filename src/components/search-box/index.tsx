    ...
    return (
        <div className='p-3 flex  flex-col gap-4'>
            {!isEmpty(items.products) && <div>
                ...
                {items.products.map((item, index) => <Link key={index} className=' flex gap-3 w-full font-extrabold' href={`/product-detail/${item.product_model}`} onClick={onClick}>
            ...
            {!isEmpty(items.category
                    ...
                    <div className=' text-sm p-4 text-blue-400'>دسته بندی ها </div>
                    {items.category
                        .map((item, index) => <Link key={index} className=' flex gap-3 w-full font-extrabold' href={`/${item.id}`} onClick={onClick}>
                            <Icon icon="iconamoon:search-thin" width="24" height="24" className=' font-extrabold' />
                            <h1>{item.title}</h1>
                        </Link>)}
                </div>}

            {isEmpty(items.catergory) && isEmpty(items.products) && <div className=' w-full  text-gray-500 justify-center items-center'> نتیجه ای یافت نشد</div>}
        </div >
    )
}
