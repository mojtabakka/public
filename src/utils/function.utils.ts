...
export function addCommasSeprator(nStr: string) {
  nStr += "";
  const x = nStr.split(".");
  let x1 = x[0];
...
export function convertMiladiDateToJalaliDate(date = new Date().toISOString()) {
  return moment(new Date(date).toISOString(), "YYYY-M-D ").format(
    "jYYYY/jM/jD"
  );
}

export function getMonthName(monthNumber: string | number): string | undefined {
  ...
  const monthKey = Number(monthNumber);
  if (isNaN(monthKey) || monthKey < 1 || monthKey > 12) {
...
export function getCompleteDateToPersian(date: Date | string, jalaali = false) {
...
export function changeMaskValueToNumber(val: string) {
...
export function englishToPersianNumbers(text: string | number): string {
